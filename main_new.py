...
async def test(ctx, arg):
    if bot_config[str(ctx.guild.id)]['commands']['test']['enabled']:
        await ctx.send(arg)
    else:
        await ctx.send('This command is disabled in this channel', delete_after = 5)

@bot.command(description = '\"Hello\" in English', brief = '\"Hello\" in English', help = 'Returns \"Hello\" in English')
async def hello(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['hello']['enabled']:
...
async def hello_german(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['hello_german']['enabled']:
...
async def hello_russian(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['hello_russian']['enabled']:
...
async def guten(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['guten']['enabled']:
        await ctx.send('tag')
    else:
        await ctx.send('This command is disabled in this channel', delete_after = 5)
...
async def rickroll(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['rickroll']['enabled']:
...
async def mods_statistics(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['mods_statistics']['enabled']:
...
async def ping(ctx):
    if bot_config[str(ctx.guild.id)]['commands']['ping']['enabled']:
        await ctx.send('pong')
    else:
        await ctx.send('This command is disabled in this channel', delete_after = 5)
...
async def clearchat(ctx, arg: int):
    if bot_config[str(ctx.guild.id)]['commands']['clearchat']['enabled']:

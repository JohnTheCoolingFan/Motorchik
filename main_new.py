...
async def list_config(ctx):
    ...
    config_commands_embed = '\n'.join([('**`'+command.name+'`**\n'+((('Whitelisted in:\n'+'\n'.join(['<#'+str(whitelist_id)+'>' for whitelist_id in bot_guild_com_cfg[command.name]['whitelist']])) if len(bot_guild_com_cfg[command.name]['whitelist']) != 0 else (('Blacklisted in:\n'+'\n'.join(['<#'+str(blacklist_id)+'>' for blacklist_id in bot_guild_com_cfg[command.name]['blacklist']])) if len(bot_guild_com_cfg[command.name]['blacklist']) != 0 else 'No channel filter')) if bot_guild_com_cfg[command.name]['enabled'] else 'Disabled')) for command in bot.commands])

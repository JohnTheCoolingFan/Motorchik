...
MOD_LIST = ['Random Factorio Things', 'Plutonium Energy', 'RealisticReactors Ingo', 'Placeable-off-grid',
            'No Artillery Map Reveal']
DEFAULT_GUILD_CONFIG = {'name': '', 'welcome_channel_id': 0, 'welcome_enabled': True, 'log_channel_id': 0,
                        'log_enabled': True, 'default_roles': [], 'commands': {}}
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(
            '**{0.author.mention}, this command is disabled in this channel or on this server.**'.format(ctx),
            delete_after=5)
    else:
...
async def on_member_join(member):
    if bot_config[str(member.guild.id)]['welcome_enabled']:
        await member.guild.get_channel(bot_config[str(member.guild.id)]['welcome_channel_id']).send(
            'Welcome, {0.mention}'.format(member))
    if len(bot_config[str(member.guild.id)]['default_roles']) != 0:
        await member.add_roles(
            *[member.guild.get_role(role_id) for role_id in bot_config[str(member.guild.id)]['default_roles']],
            reason='Default roles given on member join.')
...
async def on_member_remove(member):
    if bot_config[str(member.guild.id)]['welcome_enabled']:
        await member.guild.get_channel(bot_config[str(member.guild.id)]['welcome_channel_id']).send(
            'Goodbye, {0.mention}!'.format(member))
...
@bot.command(aliases=['gutentag'], description='\"Hello\" in German', brief='\"Hello\" in German',
             help='Returns \"Hello\" in German')
@commands.check(is_enabled)
async def hello_german(ctx):
    await ctx.send('Guten tag')


@bot.command(aliases=['privet'], description='\"Hello\" in Russian', brief='\"Hello\" in Russian',
             help='Returns \"Hello\" in Russian')
@commands.check(is_enabled)
...
@bot.command(aliases=['modstat'], description='Info about mods', brief='Info about mods',
             help='Prints a bunch of commands for uBot to display info about mods')
@commands.check(is_enabled)
...
@bot.command(aliases=['clear'], description='Clear chat', brief='Clear chat',
             help='Deletes specified count of messages in this channel. Only for admins and moderators.')
@commands.check(is_enabled)
...
async def whitelist(ctx, command_name):
    if ctx.author.permissions_in(ctx.channel).administrator:
        if bot_config[str(ctx.guild.id)]['commands'].get(command_name):
            bot_config[str(ctx.guild.id)]['commands'][command_name]['whitelist'] = list(
                set([channel.id for channel in ctx.message.channel_mentions]))
            await write_config()
...
async def blacklist(ctx, command_name):
    if ctx.author.permissions_in(ctx.channel).administrator:
        if bot_config[str(ctx.guild.id)]['commands'].get(command_name):
            bot_config[str(ctx.guild.id)]['commands'][command_name]['blacklist'] = list(
                set([channel.id for channel in ctx.message.channel_mentions]))
            await write_config()
...
async def welcome_channel(ctx):
    if ctx.author.permissions_in(ctx.channel).administrator:
        if not len(list(set([channel.id for channel in ctx.message.channel_mentions]))) > 1:
            bot_config[str(ctx.guild.id)]['welcome_channel_id'] = \
                list(set([channel.id for channel in ctx.message.channel_mentions]))[0]
            await write_config()
...
async def log_channel(ctx):
    if ctx.author.permissions_in(ctx.channel).administrator:
        if not len(list(set([channel.id for channel in ctx.message.channel_mentions]))) > 1:
            bot_config[str(ctx.guild.id)]['log_channel_id'] = \
                list(set([channel.id for channel in ctx.message.channel_mentions]))[0]
            await write_config()
...
async def enable_welcome(ctx):
    if ctx.author.permissions_in(ctx.channel).administrator:
        ...
        await write_config()
        await ctx.send(
            'Welcome messsages in channel <#{0}> enabled.'.format(bot_config[str(ctx.guild.id)]['welcome_channel_id']))
    else:
        await ctx.send('You are not allowed to use `config` command')
...
async def disable_welcome(ctx):
    if ctx.author.permissions_in(ctx.channel).administrator:
        ...
        await write_config()
        await ctx.send(
            'Welcome messsages in channel <#{0}> disabled.'.format(bot_config[str(ctx.guild.id)]['welcome_channel_id']))
    else:
        await ctx.send('You are not allowed to use `config` command')
...
async def enable_log(ctx):
    if ctx.author.permissions_in(ctx.channel).administrator:
        ...
        await write_config()
        await ctx.send(
            'Log messsages in channel <#{0}> enabled.'.format(bot_config[str(ctx.guild.id)]['log_channel_id']))
    else:
        await ctx.send('You are not allowed to use `config` command')
...
async def disable_log(ctx):
    if ctx.author.permissions_in(ctx.channel).administrator:
        ...
        await write_config()
        await ctx.send(
            'Log messsages in channel <#{0}> disabled.'.format(bot_config[str(ctx.guild.id)]['log_channel_id']))
    else:
        await ctx.send('You are not allowed to use `config` command')
...
async def list_config(ctx):
    ...
    config_embed.add_field(name='Default roles', value='\n'.join(
        ['<@&{0}>'.format(roleid) for roleid in bot_config[str(ctx.guild.id)]['default_roles']]), inline=False)
    config_embed.add_field(name='Welcome messages',
                           value='<#{0}>'.format(bot_config[str(ctx.guild.id)]['welcome_channel_id']) if
                           bot_config[str(ctx.guild.id)]['welcome_enabled'] else 'Disabled', inline=True)
    config_embed.add_field(name='Log messages',
                           value='<#{0}>'.format(bot_config[str(ctx.guild.id)]['log_channel_id']) if
                           bot_config[str(ctx.guild.id)]['log_enabled'] else 'Disabled')
    config_commands_embed = '\n'.join([('**`{0.name}`**'.format(command) + ('\n{0}'.format(((('Blacklisted in:' + (
        '\n'.join(['<#{0}>'.format(blacklist_channel) for blacklist_channel in
                   bot_config[str(ctx.guild.id)]['commands'][command.name]['blacklist']]))) if len(
        bot_config[str(ctx.guild.id)]['commands'][command.name]['whitelist']) == 0 else ('Whitelisted in:' + ('\n'.join(
            ['<#{0}>'.format(whitelist_channel) for whitelist_channel in
             bot_config[str(ctx.guild.id)]['commands'][command.name]['whitelist']])))) if (
        len(bot_config[str(ctx.guild.id)]['commands'][command.name]['blacklist']) != 0 or len(
            bot_config[str(ctx.guild.id)]['commands'][command.name]['whitelist']) != 0) else 'Allowed in all channels') if
        bot_config[
        str(ctx.guild.id)][
        'commands'][
        command.name][
    ...
    for guild_id, guild_config in bot_config.items():
        for command in bot.commands:
            if not command.name in guild_config['commands'].keys():
                print(
                    'Config for command "%s" not found in config of guild "%s"' % (command.name, guild_config['name']))

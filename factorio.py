...
class FactorioCog(commands.Cog, name='Factorio'):
    ...
    async def new_mods_statistics(self, ctx, mod_name):
        ...
        if request.status_code != 404:
            await ctx.send('https://mods.factorio.com'+sorted(request.json()['releases'], key=lambda release: release['version'], reverse=True)[0]['download_url']+'\nOR\n'+'https://factorio-launcher-mods.storage.googleapis.com/'+mod_name+'/'+sorted(request.json()['releases'], key=lambda release: release['version'], reverse=True)[0]['version']+'.zip')
        else:

...
class FactorioCog(commands.Cog, name='Factorio'):
    ...
    async def newer_mods_statistics(self, ctx: commands.Context, *mods_names):
        ...
        if mods_data:
            ...
            embed = discord.Embed(title='Mod not found', description='Failed to find mods', color=discord.Color.from_rgb(255, 10, 10))
            await ctx.send(embed=embed)

    async def send_mod_embed(self, ctx: commands.Context, mod_data: dict):
        embed = discord.Embed(title=mod_data['title'], description=mod_data['description'], url=mod_data['url'], timestamp=mod_data['timestamp'], color=mod_data['color'])
        ...
        embed.add_field(name='Download', value='[From official mod portal]({})\n[From Factorio Launcher storage]({})'.format(mod_data['download_official'], mod_data['download_launcher']))
        ...
        embed.add_field(name='Author', value='[{author}](https://mods.factorio.com/user/{author})'.format(author=mod_data['author']))
        await ctx.send(embed=embed)
